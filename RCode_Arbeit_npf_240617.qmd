...
#Serafin
records_json_S <- jsonlite::read_json("C:/Users/Nads/OneDrive - ZHAW/General/Projektarbeit/Ausgangsdaten/Serafin/Takeout_Serafin/Standortverlauf (Zeitachse)/Records.json", simplifyVector = TRUE)
records_S <- records_json_S[[1]] |> 
  mutate( Person = "Serafin")


#Nadja
records_json_N <- jsonlite::read_json("C:/Users/Nads/OneDrive - ZHAW/General/Projektarbeit/Ausgangsdaten/Nadja/Takeout/Standortverlauf (Zeitachse)/Records.json", simplifyVector = TRUE)
...
joined_data <- joined_data %>%
  mutate(Distance = sqrt((lon_copy.Serafin - lon_copy.Nadja)^2 + 
                           (lat_copy.Serafin - lat_copy.Nadja)^2))
...
joined_data <- joined_data %>%
  mutate(
    Distance_km = 2 * earth_radius * asin(sqrt(
      sin((lat_copy.Nadja - lat_copy.Serafin) * pi / 180 / 2)^2 +
      cos(lat_copy.Serafin * pi / 180) * cos(lat_copy.Nadja * pi / 180) *
      sin((lon_copy.Nadja - lon_copy.Serafin) * pi / 180 / 2)^2
    ))
  )
...
meeting_table <- meets_data %>%
  mutate(Meeting_Longitude = (lon_copy.Serafin + lon_copy.Nadja) / 2,
         Meeting_Latitude = (lat_copy.Serafin + lat_copy.Serafin) / 2) %>%
...
sf_meeting_table <- st_as_sf(meeting_table, coords = c("Meeting_Longitude", "Meeting_Latitude"), crs = 4326)
final_plot <- ggplot() +
  geom_sf(data = osm_map$osm_lines, color = "grey80", size = 0.3) +
  geom_sf(data = sf_meeting_table, aes(color = Formatted_Time), size = 2) +
  scale_color_manual(values = c("#FF5733", "#33C1FF", "#D2FF33", "#9933FF"),
                     name = "Treffzeitpunkt",
...
# Clean up activity data to remove list syntax and consolidate activity names
all_records <- all_records %>%
  mutate(activity1 = gsub('list\\(activity = |\\)', '', activity1)) %>%
  mutate(activity1 = gsub('"', '', activity1)) %>%
  mutate(activity1 = gsub("list\\(type = ", "", activity1))
...
p1 <- ggplot(dist_by_period_year, aes(x = period, y = total_distance, fill = factor(year))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = brewer.pal(n = 3, name = "Set3")[c(1, 2, 4)]) +
  labs(
    title = "Verteilung der zurückgelegten Distanz nach Tageszeit und Jahr",
    x = "Tageszeit",
    y = "Distanz (km)",
...
# Erstellen einer Sequenz aller Tage im Jahr 2024
all_dates <- seq.Date(from = as.Date("2024-01-01"), to = as.Date("2024-12-31"), by = "day")

# Zusammenführen der Daten mit allen Tagen im Jahr 2024, fehlende Tage mit 0 km auffüllen
trend_2024_daily <- data.frame(date = all_dates) %>%
  left_join(trend_2024_daily, by = "date") %>%
  mutate(total_distance = ifelse(is.na(total_distance), 0, total_distance))

# Plot erstellen
p5_daily <- ggplot(trend_2024_daily, aes(x = date, y = total_distance)) +
  geom_point(color = "black") +  # Schwarze Punkte für jeden Tag
  geom_smooth(method = "lm", se = FALSE, color = "blue") +  # Lineare Regression hinzufügen
  labs(
    title = "Trend der zurückgelegten Distanz über die Tage hinweg (2024)",
    x = "Datum",
    y = "Distanz (km)"
  ) +
  theme_minimal() +
...
correlation_table <- data.frame(
  Category = c(rep("Month-Distance Correlation", nrow(cor_by_year)),
               rep("Period-Distance Correlation", nrow(cor_period_by_year)),
               "Overall Month-Distance Correlation",
               "Overall Activity-Distance Correlation"),
  Year = c(cor_by_year$year, cor_period_by_year$year, NA, NA),
  Correlation = c(cor_by_year$correlation, cor_period_by_year$correlation, cor_overall_month$correlation, cor_overall_activity$correlation)
)

# Speicherort der CSV-Datei
file_path <- "C:/Users/Nads/OneDrive - ZHAW/General/Projektarbeit/Bericht/Tabelle/correlation_table.csv"

# Speichern der Tabelle als CSV
write.csv(correlation_table, file = file_path, row.names = FALSE)
...
# Daten für Nadja von Januar 2022 bis heute vorbereiten
nadjas_data <- records_N %>%
  mutate(
    timestamp = as.POSIXct(timestamp, format = "%Y-%m-%dT%H:%M:%S", tz = "UTC"),
...
daily_period_record_count <- nadjas_data %>%
  group_by(period) %>%
...
season_period_record_count <- nadjas_data %>%
...
season_record_count <- nadjas_data %>%
  group_by(season) %>%
  summarize(record_count = n(), .groups = 'drop')
...
period_record_count <- nadjas_data %>%
  group_by(period) %>%
  summarize(record_count = n(), .groups = 'drop')
...
# Reihenfolge der Tageszeiten definieren
levels_order <- c("Morgen (06-10 Uhr)", "Mittag (10-14 Uhr)", "Nachmittag (14-18 Uhr)", "Abend (18-22 Uhr)", "Nacht (22-06 Uhr)")
nadjas_data$period <- factor(nadjas_data$period, levels = levels_order)
...
# Daten für Nadja von Januar 2023 bis heute vorbereiten
nadjas_data <- records_N %>%
  mutate(
    timestamp = as.POSIXct(timestamp, format = "%Y-%m-%dT%H:%M:%S", tz = "UTC"),
    year = year(timestamp),
...
month_period_record_count <- nadjas_data %>%
  group_by(month, period) %>%
  summarize(record_count = n(), .groups = 'drop')
...
# Daten für Nadja im April 2024 vorbereiten
nadjas_data <- records_N %>%
  mutate(
    timestamp = as.POSIXct(timestamp, format = "%Y-%m-%dT%H:%M:%S", tz = "UTC"),
    year = year(timestamp),
...
day_period_record_count <- nadjas_data %>%
  group_by(day, period) %>%
  summarize(record_count = n(), .groups = 'drop')
...
# Daten für Serafin im April 2024 vorbereiten
serafins_data <- records_S %>%
  mutate(
    timestamp = as.POSIXct(timestamp, format = "%Y-%m-%dT%H:%M:%S", tz = "UTC"),
    year = year(timestamp),
    month = month(timestamp),
...
# Reihenfolge der Tageszeiten definieren
levels_order <- c("Morgen (06-10 Uhr)", "Mittag (10-14 Uhr)", "Nachmittag (14-18 Uhr)", "Abend (18-22 Uhr)", "Nacht (22-06 Uhr)")
serafins_data$period <- factor(serafins_data$period, levels = levels_order)

# Datenanalyse für die Anzahl der Aufzeichnungen nach Tageszeit und Tag
day_period_record_count <- serafins_data %>%
  group_by(day, period) %>%
  summarize(record_count = n(), .groups = 'drop')
...
ggplot(day_period_record_count, aes(x = day, y = record_count, fill = period, group = period)) +
  ...
  scale_fill_brewer(palette = "Set3") +
  labs(
    title = "Anzahl der Aktivitäten pro Tageszeit und Tag Serafin (April 2024)",
    subtitle = "Aufgeteilt nach Morgen, Mittag, Nachmittag, Abend und Nacht",
    x = "Tag",
    y = "Anzahl der Aktivitäten",
    fill = "Tageszeit"
...
nadja_day_period_record_count <- nadjas_data %>%
  group_by(day, period) %>%
  summarize(record_count = n(), .groups = 'drop') %>%
  mutate(person = "Nadja")

serafin_day_period_record_count <- serafins_data %>%
  group_by(day, period) %>%
  summarize(record_count = n(), .groups = 'drop') %>%
  mutate(person = "Serafin")

# Kombinieren der Datensätze
combined_data <- bind_rows(nadja_day_period_record_count, serafin_day_period_record_count)
...
ggplot(combined_data, aes(x = day, y = record_count, fill = period, group = period)) +
  ...
  labs(
    title = "Anzahl der Aktivitäten pro Tageszeit und Tag (April 2024)",
    subtitle = "Vergleich zwischen Nadja und Serafin",
    x = "Tag",
    y = "Anzahl der Aktivitäten",
    fill = "Tageszeit"
  ) +
...
# seasonal_data_count <- nadjas_data %>%
#   group_by(season) %>%
...
#   labs(
#     title = "Anzahl der Datensätze von Nadja nach Jahreszeit",
...
# total_duration_per_season <- nadjas_data %>%
#   group_by(season) %>%
...
#   geom_col() +
#   labs(
#     title = "Gesamtdauer der Bewegung von Nadja nach Jahreszeit in Stunden",
#     subtitle = "Datenanalyse ab Januar 2022",
...
# monthly_data_summary <- nadjas_data %>%
#   group_by(month) %>%
#   summarize(
#     count = n(),
...
#   ) +
#   labs(
#     title = "Monatliche Analyse der Bewegungsdaten von Nadja",
#     subtitle = "Von Januar 2022 bis heute",
#     x = "Monat",
...
# }
# 
# # Daten für Serafin von Januar 2022 bis heute vorbereiten
# serafins_data <- records_S %>%
#   mutate(
#     timestamp = as.POSIXct(timestamp, format = "%Y-%m-%dT%H:%M:%S", tz = "UTC"),
#     month = month(timestamp),
...
# 
# # Daten nach Jahreszeit gruppieren und Anzahl der Datensätze pro Saison zählen
# seasonal_data_count <- serafins_data %>%
#   group_by(season) %>%
#   summarize(count = n(), .groups = 'drop')
# 
# # Median der Gesamtzählungen für jede Jahreszeit berechnen
...
#   scale_fill_brewer(palette = "Set1", name = "Jahreszeit") +
#   labs(
#     title = "Anzahl der Datensätze von Serafin nach Jahreszeit",
#     subtitle = "Datenanalyse ab Januar 2022",
#     y = "Anzahl der Datensätze",
#     x = "Jahreszeit",
#     caption = "Jahreszeiten: Frühling (März-Mai), Sommer (Juni-Aug), Herbst (Sept-Nov), Winter (Dez-Feb)"
...
# 
# # Gesamtdauer pro Jahreszeit berechnen
# total_duration_per_season <- serafins_data %>%
#   group_by(season) %>%
#   summarize(total_duration = sum(duration), .groups = 'drop')
# 
# # Plot der Gesamtdauer der Bewegung pro Jahreszeit in Stunden
...
#   geom_col() +
#   labs(
#     title = "Gesamtdauer der Bewegung von Serafin nach Jahreszeit in Stunden",
#     subtitle = "Datenanalyse ab Januar 2022",
#     y = "Gesamtdauer (Stunden)",
#     x = "Jahreszeit",
#     caption = "Jahreszeiten: Frühling (März-Mai), Sommer (Juni-Aug), Herbst (Sept-Nov), Winter (Dez-Feb)"
...
# # Daten nach Monat gruppieren und Anzahl der Datensätze pro Monat sowie Gesamtdauer zählen
# monthly_data_summary <- serafins_data %>%
#   group_by(month) %>%
#   summarize(
#     count = n(),
#     total_duration = sum(duration),
...
#   geom_text(aes(x = 1, y = max_count, label = sprintf("Korrelation: %.2f", correlation_result)), hjust = -0.1, vjust = -0.5, color = "black") +
#   labs(
#     title = "Monatliche Analyse der Bewegungsdaten von Serafin",
#     subtitle = "Von Januar 2022 bis heute",
#     x = "Monat",
#     caption = "Blaue Balken zeigen die Anzahl der Datensätze, rote Linie zeigt die skalierte Gesamtdauer."
#   ) +
...
# 
# # Daten für Nadja und Serafin vorbereiten
# nadjas_data <- records_N %>%
#   mutate(
#     Person = "Nadja",
#     timestamp = as.POSIXct(timestamp, format = "%Y-%m-%dT%H:%M:%S", tz = "UTC"),
...
# 
# # Daten kombinieren
# combined_data <- bind_rows(nadjas_data, serafins_data)
# 
...
# if ("Nadja_total_duration" %in% names(comparison_data) && "Serafin_total_duration" %in% names(comparison_data)) {
#   correlation_duration <- cor(comparison_data$Nadja_total_duration, comparison_data$Serafin_total_duration, use = "complete.obs")
#   correlation_count <- cor(comparison_data$Nadja_count, comparison_data$Serafin_count, use = "complete.obs")
# 
#   # Ergebnisse ausgeben
#   print(sprintf("Korrelation der Gesamtdauer zwischen Nadja und Serafin: %.2f", correlation_duration))
#   print(sprintf("Korrelation der Anzahl der Datensätze zwischen Nadja und Serafin: %.2f", correlation_count))
# } else {
...
#   scale_fill_brewer(palette = "Set1", name = "Person") +
#   labs(
#     title = "Vergleich der Aktivitätsdaten von Nadja und Serafin nach Jahreszeit",
#     y = "Gesamtdauer (Stunden)",
#     x = "Jahreszeit",
